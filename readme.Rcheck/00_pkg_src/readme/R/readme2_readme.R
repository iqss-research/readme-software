...
utils::globalVariables(c(
  "FinalParams_list", "IL_input", "IL_mu_b", "IL_sigma2_b", "L2_squared_clipped",
  "applyGradients_updateRate", "dfm_", "grab_learnRate", "init1", "init2",
  "readme_graph", "set_inverse_learn_action", "setclip_action", "textrpp_initialize",
  "textEmbed"
))
...
#'
#' @param minMatch Minimum number of labeled documents per category to include via KNN matching. Default is 8.
#'
#' @param diagnostics Should diagnostics be returned? Boolean, default is FALSE.
#'
#' @param sigma_ep Small constant added to variance for numerical stability. Default is 0.
#'
#' @param regraph Should the TensorFlow graph be rebuilt? Boolean, default is FALSE.
#'
#' @param conda_env Name of the conda environment to use for TensorFlow. Default is NULL (uses default environment).
#'
#' @param otherOption Reserved for future use. Default is NULL.
#'
#' @param wt_catDistinctiveness Weight for category distinctiveness in the loss function. Default is NULL (uses default weighting).
#'
#' @param wt_featDistinctiveness Weight for feature distinctiveness in the loss function. Default is NULL (uses default weighting).
#'
#' @param tensorflowSeed Random seed for TensorFlow reproducibility. Default is NULL.
#'
...
readme <- function(dfm ,
  ...
  try(eval(parse(text="require('tensorflow',quietly=TRUE)"),envir = globalenv()),TRUE)
  if(!is.null(conda_env)){
    eval(parse(text=sprintf("(try(tensorflow::use_condaenv(condaenv='%s',required = TRUE), TRUE))",
                          conda_env)),envir = globalenv())
  }
  eval(parse(text="suppressWarnings(try(tensorflow::use_compat(version='v1'), TRUE))"),envir = globalenv())
  ...
  if("list" %in% dfm_class){
    dfm_labeled = try(as.matrix(data.table::fread(cmd = dfm$labeled_cmd))[,-1],TRUE)
  ...
  regraph_ = try((ncol(IL_input) != ncol(dfm_labeled)), TRUE)
  if(("try-error" %in% class(regraph_)) | regraph_ == TRUE){regraph_ <- TRUE;
      try(suppressWarnings(rm(readme_graph)),TRUE)
  }
  ...
  try(source(graphSource,local=FALSE),TRUE)
  try(unlink(graphSource),TRUE);
  ...
  MultMat_tf_v          = t(do.call(rbind,sapply(1:nCat,function(x){
    ...
    for(iter_i in 1:nBoot){
          ...
          try(S_$close(), TRUE)
          try(tf$keras$backend$clear_session(), TRUE)
          try(tf$keras$backend$reset_uids(), TRUE)
  ...
  for(iter_i in 1:nBoot){
    ...
    if(justTransform == FALSE){
      ...
      BOOTSTRAP_EST = sapply(1:nbootMatch, function(boot_iter){
        ...
        est_obsMatch = function(weight_indices){
          ...
          ED_sampled_averaged = try(colMeans(do.call(rbind,est_readme2_[1,])), TRUE)
    ...
    if(justTransform==TRUE){
      ### Calculate the transformed DFM
      transformed_dfm <- matrix(NA, nrow =  length(labeledIndicator), ncol = nProj)
      test_ = try(transformed_dfm[which(labeledIndicator==1),] <- apply(out_dfm_labeled, 2, f2n), TRUE)
      test__ = try(transformed_dfm[which(labeledIndicator==0),] <- apply(out_dfm_unlabeled, 2, f2n), TRUE)
  ...
  if(verbose==TRUE){ cat("Finished!") }
  ## Parse output
  ## If no diagnostics wanted
  #sort( sapply(ls(),function(x){object.size(get(x))}))
  return( list(point_readme = colMeans(boot_readme, na.rm = TRUE),
               point_readme_NoMatching = colMeans(boot_readme_NoMatching, na.rm = TRUE),
...
graph_file_gen <- function(nDim,nProj=20,NObsPerCat=10,
                           wt_catDistinctiveness=NULL,
                           wt_featDistinctiveness=NULL,
                           regraph = FALSE,use_env=globalenv(),TF_SEED=NULL){
  {
  seed_text <- ";"
  ...
  if(  (!"readme_graph" %in% ls(envir = globalenv())) | regraph == TRUE){
    if(regraph == TRUE){
