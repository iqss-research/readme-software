...
readme <- function(dfm, labeledIndicator, categoryVec, 
  ...
  browser() 
  dropout_rate1 = tf$reshape(0.60*tf$nn$sigmoid(tf$Variable(rep(0, times = nDim), trainable = T, dtype = tf$float32))+0.20, list(nDim, 1L) )##RATE FOR DROPPING NODES 
  ...
  dropout_rate_s = 0.5
  ulim1_s         = -0.5 * (1-dropout_rate_s) / ( (1-dropout_rate_s)-1)
  MASK_VEC1_2     = tf$multiply(tf$nn$relu(tf$sign(tf$random_uniform(list(nDim,1L),-0.5,ulim1_s))), 1 / (ulim1_s/(ulim1_s+0.5)))
  ...
  ad_obj        = -tf$reduce_mean( tf$abs(LFinal_ - LFinal) )
