...
cleanme <- function(my_text,finalEncoding = "ASCII"){ 
  ...
  my_text <- sapply(my_text, 
                    function(x){ startingEncoding <- Encoding(x)
                      if(startingEncoding == "unknown"){ x<-iconv(x, from = finalEncoding, to = finalEncoding, sub = '', mark = TRUE) } 
                      if(startingEncoding != "unknown"){ x<-iconv(x, from = startingEncoding, to = finalEncoding, sub = '', mark = TRUE) }
                      return(x) })
  names(my_text) <- NULL 
  
  ## Remove HTML/XML tags
  my_text <- gsub(my_text, pattern = "<.*?>", replacement = " <html> ") 
  
  ## Remove newline, carriage return and tabs
  my_text <- gsub(my_text, pattern = "\n", replacement = " ") 
  my_text <- gsub(my_text, pattern = "\r", replacement = " ") 
  my_text <- gsub(my_text, pattern = "\t", replacement = " ") 
  
  ## Convert URLs to generic <url> feature
  url_pattern1 <- "http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+"
  url_pattern2 <- " ?(f|ht)(tp)(s?)(://)(.*)[.|/](.*)"
  my_text <- gsub(my_text, pattern = url_pattern1, replacement = " <url> ") 
  my_text <- gsub(my_text, pattern = url_pattern2, replacement = " <url> ") 
  
  ## Convert standard emoticons
  my_text <- gsub(my_text, pattern = ":-\\)", replacement = " <smile> ")
  my_text <- gsub(my_text, pattern = "\\^_\\^", replacement = " <smile> ")
  my_text <- gsub(my_text, pattern = ":\\)", replacement = " <smile> ")
  my_text <- gsub(my_text, pattern = "\\(:", replacement = " <smile> ")
  my_text <- gsub(my_text, pattern = "\\(-:", replacement = " <smile> ")
  my_text <- gsub(my_text, pattern = ":D", replacement = " <smile> ")
  my_text <- gsub(my_text, pattern = ":-\\(", replacement = " <sadface> ")
  my_text <- gsub(my_text, pattern = ":\\(", replacement = " <sadface> ")
  my_text <- gsub(my_text, pattern = "\\):", replacement = " <sadface> ")
  my_text <- gsub(my_text, pattern = "\\)-:", replacement = " <sadface> ")
  my_text <- gsub(my_text, pattern = "D:", replacement = " <sadface> ")
  
  ## Remove punctuation
  my_text <- gsub(my_text, pattern = "\\s*-\\B|\\B-\\s*", replacement = " ") 
  my_text <- gsub(my_text, pattern = " \\- ", replacement = " ") 
  my_text <- gsub(my_text, pattern = "\\--", replacement = " ") 
  my_text <- gsub(my_text, pattern = "@\\S+", replacement = " <user> ") ## Twitter-specific, any @text converted to a generic <user> feature
  
  my_text <- gsub(my_text, pattern = "[[:digit:]]+", replacement = " <number> ") ## Convert numbers to generic <number> feature
  ...
  my_text <- gsub(my_text, pattern = '\\"', replacement = " ")
  my_text <- gsub(my_text, pattern = '\\\u00A8', replacement = " ")
  my_text <- gsub(my_text, pattern = "\\\u00F4", replacement = " ") 
  ...
  my_text <- gsub(my_text, pattern = "\\!", replacement = " ! ")
  my_text <- gsub(my_text, pattern = "\\?", replacement = " ? ")
  my_text <- gsub(my_text, pattern = "\\\u00F5", replacement = " ' ")
  my_text <- gsub(my_text, pattern = "\\\u00F5", replacement = " ' ")
  
  ## Contractions are split out
  my_text <- gsub(my_text, pattern = "\\'s", replacement = " \\'s ")
  my_text <- gsub(my_text, pattern = "n\\'t ", replacement = " n\\'t ")
  my_text <- gsub(my_text, pattern = "\\'d ", replacement = " \\'d ") 
  my_text <- gsub(my_text, pattern = "\\'re ", replacement = " \\'re ") 
  my_text <- gsub(my_text, pattern = "\\'ve ", replacement = " \\'ve ") 
  my_text <- gsub(my_text, pattern = "\\'ll ", replacement = " \\'ll ") 
  my_text <- gsub(my_text, pattern = "\\'m ", replacement = " \\'m ") 
  
  ## Standard abbreviations
  my_text <- gsub(my_text, pattern = "gov\\'t", replacement = " government ") 
  
  ### Non-separating punctuation
  my_text <- gsub(my_text, pattern = "%", replacement = "") 
  my_text <- gsub(my_text, pattern = ">>", replacement = "") 
  my_text <- gsub(my_text, pattern = "<<", replacement = "") 
  my_text <- gsub(my_text, pattern = "__", replacement = "") 
