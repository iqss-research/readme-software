...
readme <- function(dfm, labeledIndicator, categoryVec, 
  ...
  for(ape in 1:nCat){ 
      ...
      eval(parse(text = sprintf("d_%s = tf$data$Dataset$from_tensor_slices(dfm_labeled[l_indices_by_cat[[ape]],])$`repeat`()$shuffle(as.integer( (nboot*sgd_iters)/length(l_indices_by_cat[[ape]]) ))$batch(NObsPerCat)", ape)) )
