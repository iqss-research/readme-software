...
readme <- function(dfm, labeledIndicator, categoryVec, 
  ...
  for(iter_i in 1:nboot){ 
      ...
      for(awer in 1:sgd_iters){
        ...
        if(awer %% 3 == 0){ nrestart=nrestart+1;sess$run(inverse_learning_rate$assign( abs(inverse_learning_rate-abs(rnorm(1,mean=0, sd = 1/abs(inverse_learning_rate_vec[awer-1])) ) ))) }
        print(inverse_learning_rate_vec[awer])
      }
