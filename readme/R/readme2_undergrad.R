...
#' `list(tokenizer_parallelism = TRUE,
...
undergrad <- function(documentText,
                      ...
                      unique_terms = TRUE, verbose = TRUE,
                      numericization_method = "vector_summaries",
                      textEmbed_control = list(tokenizer_parallelism = TRUE,
                                               model = "bert-base-multilingual-uncased",
                                               layers = -2L)){
    if(numericization_method == "transformer_based"){
      ...
      if (!requireNamespace("text", quietly = TRUE)) {
        stop("Package 'text' is required for transformer-based numericization. Install with: install.packages('text')")
      }
    ...
    if (reattempt == TRUE){
      ...
      for (indx in 1:length(reattempt_regex)){
        ...
        match_missing[is.na(match_missing)] <- match(gsub(missing_stems[is.na(match_missing)],
                                                          ...
                                                          replacement = reattempt_regex[[indx]][2]),
                                                     wordVec_terms)
